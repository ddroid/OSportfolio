...
export default function AboutWebsite() {
  return (
    <div class='bg-slate-700 text-white rounded-md pt-12 h-full w-full flex flex-col items-center text-center text-md'>
      ...
      <h1 class='font-bold text-4xl gradient-text'>{pepsimode.value ? 'Cat OS' : 'PortfoliOS'}</h1>
              <p class='font-mono text-sm opacity-50'>v1.1.2 {pepsimode.value ? ' - 🐈 Cat mode' : ''}</p>
      </div>
      ...
      <div class='flex flex-col gap-2 w-full h-full p-5 text-start text-md font-light'>
        ...
        <div class='h-full'/>
        <p class='text-sm text-center w-full text-gray-500'>Copyright © {new Date().getFullYear()} <a href='https://github.com/ddroid' target='_blank'>Ahmad Munir</a> {t('copyright')}</p>
        <div class='flex flex-row items-center justify-center w-full text-white mt-2 mb-1 p-4 gap-3 '>
          <a class='hover:bg-white hover:text-black/80 transition bg-white/20 rounded-full flex gap-2 items-center px-2 py-1' href='https://github.com/ddroid/OSportfolio' target="_blank"><Icon path={mdiGithub} size={1}/> {t('actionbuttons.srccode')}</a>
